...
import { pillarPages, pillarSlugs } from "@/data/pillarPages";
...
const PillarPage = () => {
  ...
  const combinedSchema = { ...page.schema, ...faqSchemaData };
            ...
            {page.relatedPillars.map((rpSlug) => {
              ...
              return (
                <Link
                  key={rpSlug}
                  to={`/${rpSlug}`}
                  className="block p-5 rounded-xl border border-border hover:border-primary/40 hover:bg-primary/5 transition-all"
                >
