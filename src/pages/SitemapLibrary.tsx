...
import { Globe, Search, ArrowRight, FileText, Layers, Lock, Crown } from "lucide-react";
...
const SitemapLibrary = () => {
  ...
  const { canUseLibrary, isPremium } = useUserPlan();
  ...
  const allSitemaps = [
    ...seedSitemaps.map(s => ({ domain: s.domain, slug: s.slug, totalPages: s.totalPages, maxDepth: s.maxDepth, topLevelSections: s.topLevelSections })),
    ...dbSitemaps.map(s => ({ domain: s.domain, slug: s.slug, totalPages: s.total_pages, maxDepth: s.max_depth, topLevelSections: s.top_level_sections })),
  ...
  const handleSitemapClick = (slug: string, e: React.MouseEvent) => {
    if (!canUseLibrary) {
      e.preventDefault();
      setShowPaywall(true);
    }
  ...
  const jsonLd = {
    ...
    description: "Browse visual sitemaps of popular websites and explore their architecture.",
    url: "https://no-edit.lovable.app/sitemaps",
    mainEntity: {
      "@type": "ItemList",
      numberOfItems: allSitemaps.length,
      itemListElement: allSitemaps.slice(0, 30).map((s, i) => ({
        "@type": "ListItem", position: i + 1, name: `${s.domain} Visual Sitemap`, url: `https://no-edit.lovable.app/sitemap/${s.slug}`,
      })),
    },
    breadcrumb: {
      "@type": "BreadcrumbList",
  ...
  return (
    <>
      ...
      <MainNavigation activeTab="home" onTabChange={(tab) => { if (tab === "home") navigate("/"); }} />
      <main id="main-content" className="min-h-screen bg-background pt-14">
        <Breadcrumbs items={[{ label: "Sitemap Library" }]} />
        <div className="max-w-6xl mx-auto px-6 py-12">
          <header className="text-center mb-10">
            <h1 className="text-4xl font-bold text-foreground mb-3">Website Structure Library</h1>
            ...
            {!isPremium && (
              <div className="mt-4 inline-flex items-center gap-2 text-sm text-muted-foreground bg-muted/50 px-4 py-2 rounded-full">
                ...
                <span>Upgrade to Pro to open and use sitemaps</span>
          ...
          <div className="max-w-md mx-auto mb-8">
            <div className="relative">
              ...
              <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search websites (e.g., stripe, figma)..." className="pl-10" />
            </div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-12">
            {filtered.map(s => (
              <Link key={s.slug} to={`/sitemap/${s.slug}`} onClick={(e) => handleSitemapClick(s.slug, e)}>
                <Card className="p-4 hover:border-primary/50 hover:shadow-md transition-all h-full relative">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="font-semibold text-sm text-foreground">{s.domain}</span>
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-center">
                    <div>
                      <FileText className="h-3.5 w-3.5 mx-auto mb-1 text-muted-foreground" />
                      <div className="text-sm font-bold text-foreground">{s.totalPages}</div>
                      <div className="text-[10px] text-muted-foreground">Pages</div>
                    </div>
                    <div>
                      <Layers className="h-3.5 w-3.5 mx-auto mb-1 text-muted-foreground" />
                      <div className="text-sm font-bold text-foreground">{s.maxDepth}</div>
                      <div className="text-[10px] text-muted-foreground">Depth</div>
                    </div>
                    <div>
                      <div className="text-sm font-bold text-foreground">{s.topLevelSections}</div>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-12">
              ...
              <Button asChild><Link to="/analyzer">Analyze this domain instead →</Link></Button>
            </div>
          )}

          <div className="text-center py-8 border-t border-border">
            <h2 className="text-2xl font-bold text-foreground mb-2">Analyze Any Website</h2>
            <p className="text-muted-foreground mb-4">Enter any domain and get a visual sitemap with real crawl data, instantly.</p>
            <Button asChild size="lg" className="gap-2">
              <Link to="/analyzer">Start Analyzing <ArrowRight className="h-4 w-4" /></Link>
            </Button>
          </div>
        </div>
        <Footer />
