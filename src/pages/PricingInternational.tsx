...
const PricingInternational = () => {
  ...
  return (
    <>
      <SEO
        title="EPIC Pro Pricing — $5/mo or $15 Lifetime | Visual Sitemap Builder"
        description="Upgrade to EPIC Pro for unlimited visual sitemaps, PDF/PNG export, UX testing, website analyzer, and more. $5/month or $15 lifetime."
        ...
        ogType="product"
        structuredData={pricingPageSchema("USD", 5)}
        hreflang={[
      ...
      <div className="min-h-screen bg-background relative">
        ...
        <div className="relative z-10 container mx-auto px-4 py-12">
          ...
          <div className="text-center mb-12">
            <h1 className="text-4xl md:text-5xl font-bold text-foreground mb-4">EPIC Pro</h1>
              ...
              <div className="space-y-6">
                <div>
                  <h2 className="text-2xl font-bold mb-2 text-yellow-500 flex items-center gap-2">
                    <Crown className="h-5 w-5" /> EPIC Pro
                  </h2>
                  <div className="grid grid-cols-2 gap-2 mb-4">
                    <button
                      onClick={() => setSelectedPlan("monthly")}
                      className={`p-3 rounded-lg border-2 text-left transition-all ${
                        selectedPlan === "monthly" ? "border-yellow-500 bg-yellow-500/5" : "border-border/30 hover:border-yellow-500/30"
                      }`}
                    >
                    ...
                    <button
                      ...
                      className={`p-3 rounded-lg border-2 text-left transition-all relative ${
                        selectedPlan === "lifetime" ? "border-yellow-500 bg-yellow-500/5" : "border-border/30 hover:border-yellow-500/30"
                      }`}
                    >
                      <span className="absolute -top-2 right-2 text-[9px] font-bold bg-yellow-500 text-black px-1.5 py-0.5 rounded-full">BEST VALUE</span>
                ...
                <ul className="space-y-3">
                  {["Unlimited visual sitemaps", "Unlimited sitemap pages", "PDF & PNG Export", "JSON Export", "UX Tester", "Analyze Features", "Website Structure Library", "All advanced features", "Future updates included"].map((f, i) => (
                    <li key={i} className="flex items-start gap-3">
                ...
                <div className="space-y-3 pt-2">
                  ...
                  <Button onClick={handleUPIPay} className="w-full bg-yellow-500 hover:bg-yellow-600 text-black font-semibold h-12 text-base">
                    <Zap className="mr-2 h-4 w-4" /> Pay with UPI — {selectedPlan === "monthly" ? "$5" : "$15"}
                  </Button>
